...
graph TD
    ...
    subgraph "Routing abstractions (Engine/Routers/)"
        ...
        FPR["FloodingPacketRouter\nimplements IPacketRouter\nbroadcast to nearest-visible top-k neighbours\n(baseline)"]
        SFR["SmartFloodingPacketRouter\nimplements IPacketRouter\ndirect if visible * filtered flood"]
        SPR["SwarmProtocolPacketRouter\nimplements IPacketRouter\nUP best-neighbor + DOWN tree-first\n(engine and WebApp default)"]
...
[`SimulationEngine`](Core/SimulationEngine.cs) is the default global clock.  
...
Packet enqueue behavior is centrally normalized in `SimulationEngine.RegisterPacket`: packets that keep constructor defaults inherit engine-level defaults (`DefaultPacketTtl`, `DefaultPacketTicksToTravel`), which lets UI and benchmark configs control global TTL and hop travel time consistently.
...
[`IPacketRouter`](Routers/Contracts/IPacketRouter.cs) is the single abstraction for all routing strategies.
...
| Class                                                               | Strategy                                                                                                                                                                   |
| ------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`SwarmProtocolPacketRouter`](Routers/SwarmProtocol/SwarmProtocolPacketRouter.cs) | **Engine default** (`SimulationEngine.Router`) and **WebApp default** (`SimulationConfig.SelectedRouter`). Uses nearest-visible top-k window (`k=10`): UP bootstrap fan-out until charge signal appears, then best-neighbor by `q_up`; DOWN uses tree-first forwarding by parent/children state. |
| [`SmartFloodingPacketRouter`](Routers/SmartFlooding/SmartFloodingPacketRouter.cs) | Uses nearest-visible top-k window (`k=10`), direct destination delivery when visible, and filtered flooding that excludes `From` and `PreviousHop`. |
| [`FloodingPacketRouter`](Routers/Flooding/PacketRouter.cs) | Baseline flooding over nearest-visible top-k window (`k=10`) with one clone per selected neighbor. |

All bundled routers share the same visibility candidate policy via `RoutingNeighborPolicy`, which keeps neighborhood fairness symmetric across protocol comparisons.

> **Note:** `SimulationEngine.Router` initialises to `SwarmProtocolPacketRouter`. The Blazor WebApp uses the same default via `SimulationConfig.SelectedRouter`, and users can switch routers at runtime.
...
Protocol-aware packet fields include:

- Routing metadata: `Version`, `PreviousHopMac`, `AdvertisedCharge`, `DecayEpochHint`.
- Secure metadata: `Direction`, `MessageType`, `OriginMac`, `DestinationMac`, `MessageId`, `Sequence`.
- End-to-end tag bytes: `Tag` (HMAC-SHA256 truncation to 16 bytes over secure-header + payload).

**Flood-clone identity fields**

Because flooding broadcasts cloned copies of a packet to nearest-visible top-k neighbors, two extra fields are stamped on `Packet` to make statistical analysis of these copies possible:
...
| Path                                                                                                       | Responsibility                                                                                                                         |
| ---------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| [`Core/SimulationEngine.cs`](Core/SimulationEngine.cs)                                                     | Global-default engine with scoped override support: tick loop, device registry, packet queue, typed events, `Router` + `NetworkBuilder` + `Topology`, packet defaults (`DefaultPacketTtl`, `DefaultPacketTicksToTravel`), packet-limit guard |
...
| [`Devices/NeighborState.cs`](Devices/NeighborState.cs)                                                     | Protocol neighbor table entry (`QUp`, `QTotal`, `LastSeenTick`, `SampleCount`) used by routing and parent selection                     |
| [`Devices/HubDevice.cs`](Devices/HubDevice.cs)                                                             | Central gateway; destination for generator traffic and source of periodic `BEACON` / `DECAY` protocol control packets                |
...
| [`Packets/BeaconPacket.cs`](Packets/BeaconPacket.cs)                                                       | Gateway BEACON broadcast packet with convergence threshold hint                                                                         |
| [`Packets/DecayPacket.cs`](Packets/DecayPacket.cs)                                                         | Gateway DECAY broadcast packet with epoch and damping factor                                                                            |
| [`Packets/PacketDirection.cs`](Packets/PacketDirection.cs)                                                 | Protocol traffic direction enum (`Up`, `Down`)                                                                                          |
| [`Packets/SwarmMessageType.cs`](Packets/SwarmMessageType.cs)                                               | Protocol message-type enum registry                                                                                                     |
| [`Packets/PacketAddress.cs`](Packets/PacketAddress.cs)                                                     | 6-byte address helpers (broadcast constant, compare/equality, GUID mapping)                                                             |
| [`Packets/ProtocolPayloads.cs`](Packets/ProtocolPayloads.cs)                                               | Payload records used by BEACON and DECAY control traffic                                                                                |
...
| [`Routers/RoutingNeighborPolicy.cs`](Routers/RoutingNeighborPolicy.cs)                                       | Shared deterministic visibility policy used by routers; selects nearest-visible top-k neighbors (default k=10) for fairness and bounded fan-out |
...
| [`Routers/Flooding/PacketRouter.cs`](Routers/Flooding/PacketRouter.cs)                                     | `FloodingPacketRouter`  -  flooding over nearest-visible top-k neighbors (baseline strategy)                                            |
| [`Routers/SmartFlooding/SmartFloodingPacketRouter.cs`](Routers/SmartFlooding/SmartFloodingPacketRouter.cs) | `SmartFloodingPacketRouter`  -  direct if destination visible; filtered flood otherwise, both on nearest-visible top-k neighbors         |
| [`Routers/SwarmProtocol/SwarmProtocolPacketRouter.cs`](Routers/SwarmProtocol/SwarmProtocolPacketRouter.cs) | `SwarmProtocolPacketRouter`  -  protocol routing on nearest-visible top-k neighbors (UP bootstrap -> charge-best, DOWN tree-first)      |
| [`Statistics/SimulationStatistics.cs`](Statistics/SimulationStatistics.cs)                                 | Event-driven global-default collector with scoped override support: ten metrics + history ring-buffer                                 |
...
| [`Benchmark/BenchmarkRunner.cs`](Benchmark/BenchmarkRunner.cs)                                             | Headless tick loop: isolated per-router engine/statistics context, applies events, optional router parallelism, returns session        |

---

## Comparing protocols on the same network

Because both `Router` and `NetworkBuilder` are hot-swappable properties on the engine, you can:
...
BenchmarkRunner.Run()    <- headless loop; one full reset+run per router
    ...
    |    2. engine.Reset() + stats.Reset()
    |    3. apply engine settings + register initial devices
    |    4. tick loop (0..DurationTicks):
    |         - fire scheduled BenchmarkEvents at matching ticks
    |         - engine.Tick()
    |    5. capture BenchmarkResult (final metrics + TickSnapshot[])
    |  optional: run routers in parallel (bounded)
    |
    v
...
- `Core/Documentation.md`  -  global engine with scoped-context support, events, packet-limit exception.
...
- `Routers/SwarmProtocol/Documentation.md`  -  `SwarmProtocolPacketRouter`.
- `Statistics/Documentation.md`  -  metrics, observable values, history ring buffer.
- `Benchmark/Documentation.md`  -  isolated headless benchmark runner and session types.
